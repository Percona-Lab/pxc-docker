...
if [[ -n ${ADDOP:-} ]];then 
   ADDOP="gmcast.segment=$SEGMENT; gcache.size=256M; $ADDOP"
else 
   ADDOP="gmcast.segment=$SEGMENT; gcache.size=256M"
fi
...
while true; do
    ...
    for s in `seq 1 $NUMC`;do 
        ...
        if [[ $stat1 != 'Primary' || $stat2 != 'Synced' || $stat3 != '0' || $stat4 != '0' ]];then 
            echo "Waiting for Dock${s} (or some other node to empty) to become really synced or primary: $stat1, $stat2, $stat3, $stat4"
            runagain=1
            break
        else 
            echo "Dock${s} is synced and is primary: $stat1, $stat2, $stat3, $stat4"
        fi
    done
...
for s in `seq 1 $NUMC`;do 
    ...
    if [[ $stat1 != 'Primary' || $stat2 != 'Synced'  ]];then 
        echo "Dock${s} seems to be not stable: $stat1, $stat2, $stat3, $stat4"
    else 
        echo "Dock${s} is synced and is primary: $stat1, $stat2, $stat3, $stat4"
    fi
done
...
while true;do 
    ...
    for s in `seq 1 $NUMC`;do 
        ...
        stat4=$(mysql -nNE -h $(docker port Dock${s} 3306 | cut -d: -f1) -P $(docker port Dock${s} 3306 | cut -d: -f2)  -u root -e "show global status like 'wsrep_local_send_queue'" 2>>$LOGDIR/mysql-Dock${s}.log | tail -1)
        if [[ $stat1 != 'Primary' || $stat2 != 'Synced'  ]];then 
            echo "FATAL: Dock${s} seems to be STILL unstable: $stat1, $stat2, $stat3, $stat4"
