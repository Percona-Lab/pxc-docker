...
docker run -P -e LD_PRELOAD=$PRELOAD --add-host repo.percona.com:10.10.9.209   -d -t -i -h Dock1 -v $COREDIR:/pxc/crash $PGALERA   --dns $dnsi --name Dock1 ronin/pxc:tarball bash -c "ulimit -c unlimited && chmod 777 /pxc/crash && $CMD $ECMD --wsrep-new-cluster --wsrep-provider-options='gmcast.segment=$SEGMENT; evs.auto_evict=3; evs.version=1;  evs.info_log_mask=0x3'" &>$LOGDIR/run-Dock1.log
...
for rest in `seq 2 $NUMC`; do
    ...
    set -x
    docker run -P -e LD_PRELOAD=$PRELOAD --add-host repo.percona.com:10.10.9.209  -d -t -i -h Dock$rest -v $COREDIR:/pxc/crash $PGALERA --dns $dnsi --name Dock$rest ronin/pxc:tarball bash -c "ulimit -c unlimited && chmod 777 /pxc/crash && $CMD $ECMD --wsrep_cluster_address=$CSTR --wsrep_node_name=Dock$rest --wsrep-provider-options='gmcast.segment=$SEGMENT; evs.auto_evict=3; evs.version=1; evs.info_log_mask=0x3'" &>$LOGDIR/run-Dock${rest}.log
    set +x
    #CSTR="${CSTR},Dock${rest}"
