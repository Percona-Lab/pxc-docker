...
docker run --add-host repo.percona.com:10.10.9.209   -d  -p 20000:53   -i -v /dev/log:/dev/log -e SST_SYSLOG_TAG=dnsmasq -v $RESFILE:/dnsmasq.res -v $HOSTSF:/dnsmasq.hosts --name dnscluster ronin/dnsmasq  &>$LOGDIR/dnscluster-run.log
...
set -x
( timeout -k9 $(( SDURATION+200 )) sysbench --test=$SDIR/$STEST.lua --db-driver=mysql --mysql-db=test --mysql-engine-trx=yes --mysql-ignore-errors=$IGERR  --mysql-table-engine=innodb --mysql-socket=$SOCKS --mysql-user=root  --num-threads=$NUMT --init-rng=on --max-requests=1870000000    --max-time=$SDURATION  --oltp_index_updates=20 --oltp_non_index_updates=20 --oltp-auto-inc=$AUTOINC --oltp_distinct_ranges=15 --report-interval=1  --oltp_tables_count=$TCOUNT run 2>&1 | tee $LOGDIR/sysbench_rw_run.txt ) &
syspid=$!
set +x
